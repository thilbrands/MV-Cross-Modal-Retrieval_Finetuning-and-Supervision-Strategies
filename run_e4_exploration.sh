#!/bin/bash
#
# Slurm-Job: E4-Exploration — Training auf 7 Genres, Evaluation auf allen 10.
# Unseen Genres (Extrapolation): Classical, Country, Hip-Hop (methodisch isolierteste nach Mean Similarity).
# Seen Genres (Training): Blues, Electronic music, Funk, Jazz, Pop music, Reggae, Rock music.
#
#SBATCH --job-name=e4_exploration
...
#   bash run_e4_exploration.sh
#   sbatch run_e4_exploration.sh
#   DATASET_RUN_NAME=2026-03-13_18-12-31_audioset sbatch run_e4_exploration.sh
#
set -e
...
# Fester Ordner für E4 — so bleiben die Heads immer am gleichen Ort auffindbar
export TRAINING_RUN_DIR="/work2/ra39oxet-DatasetAudioSetSubset/training_runs/e4_exploration"
mkdir -p "$TRAINING_RUN_DIR"
mkdir -p "/work2/ra39oxet-DatasetAudioSetSubset/logs"

# 7 Trainings-Genres (Komma-getrennt, exakt wie in train_val_test_split.csv)
export TRAIN_GENRES="Blues,Electronic music,Funk,Jazz,Pop music,Reggae,Rock music"
...
echo "  (projection_heads_*.pt, audio_encoder_*.pt, meta_*.json, *.out/.err, evaluation_output.txt, genre_breakdown.txt, genre_breakdown_plot.pdf/.png)"
