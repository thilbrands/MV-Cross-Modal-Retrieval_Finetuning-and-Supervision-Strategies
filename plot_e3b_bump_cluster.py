...
outside_vals = sim[diff_mask & ~both_in_cluster]
...
print(f"Outside cluster n={outside_vals.size:>9}  median={np.median(outside_vals):.3f}  mean={outside_vals.mean():.3f}", flush=True)

groups = [("Within cluster", within_vals, COLOR_WITHIN), ("Outside cluster", outside_vals, COLOR_OUTSIDE)]
all_vals = np.concatenate([within_vals, outside_vals])
