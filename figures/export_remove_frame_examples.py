...
Voraussetzung: Venv „ba“ (opencv-python), wie bei extract_and_embed:
  ...
  python3 figures/export_remove_frame_examples.py
...
def main() -> None:
    ...
    output_dir = Path(
        os.environ.get(
            "OUTPUT_DIR",
            str(config.resolve_plot_output_dir(run_dir) / "remove_examples"),
        )
    )
    ...
    if saved and os.environ.get("SKIP_COMBINE", "").strip() != "1":
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                ...
                _REPO_ROOT / "figures" / "combine_remove_example_strips.py",
            )
            ...
            print("Manuell: INPUT_DIR=... python3 figures/combine_remove_example_strips.py", flush=True)
