...
Voraussetzung: Venv „ba“ (opencv-python), wie bei extract_and_embed:
  ...
  python3 figures/export_keep_low_frame_examples.py
...
def main() -> None:
    ...
    output_dir = Path(
        os.environ.get(
            "OUTPUT_DIR",
            str(config.resolve_plot_output_dir(run_dir) / "keep_low_examples"),
        )
    )
    ...
    if saved and os.environ.get("SKIP_COMBINE", "").strip() != "1":
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                ...
                _REPO_ROOT / "figures" / "combine_keep_low_example_strips.py",
            )
            mod = importlib.util.module_from_spec(spec)
            assert spec.loader is not None
            spec.loader.exec_module(mod)
            n = mod.combine(output_dir, combined)
            print(f"Kombiniert: {n} Zeilen -> {combined}", flush=True)
        except Exception as exc:
            print(f"Hinweis: Kombi-Figure fehlgeschlagen ({exc}).", flush=True)
            print("Manuell: INPUT_DIR=... python3 figures/combine_keep_low_example_strips.py", flush=True)
