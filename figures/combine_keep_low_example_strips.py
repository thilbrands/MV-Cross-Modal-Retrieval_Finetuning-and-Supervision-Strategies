#!/usr/bin/env python3
"""
Kombiniert KEEP_LOW-Frame-Strips zu einer PNG/PDF-Figure (PIL).

Standard: Zeilen 1,3,9,10 im Manifest.

  INPUT_DIR=/path/to/keep_low_examples \\
  python3 figures/combine_keep_low_example_strips.py

Output: keep_low_examples_figure.png + .pdf
"""
...
DEFAULT_INPUT = Path.home() / "Desktop" / "keep_low_examples"
DEFAULT_ROWS = "1,3,9,10"
OUTPUT_BASENAME = os.environ.get("OUTPUT_BASENAME", "keep_low_examples_figure")
...
def resolve_strip_path(row: dict, input_dir: Path) -> Path | None:
    ...
    if index and video_id:
        guess = input_dir / f"{int(index):02d}_{video_id}_keep_low.png"
        if guess.is_file():
            return guess
    return None
...
def load_all_rows(input_dir: Path) -> list[dict]:
    manifest = input_dir / "keep_low_examples_manifest.csv"
...
def combine(input_dir: Path, output_path: Path) -> int:
    ...
    save_pdf_from_image(canvas, output_path.with_suffix(".pdf"))
    print(f"PDF: {output_path.with_suffix('.pdf')}", flush=True)
    return len(row_imgs)
